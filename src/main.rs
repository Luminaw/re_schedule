...
fn main() {
    ...
    println!("Loaded {} targets. Starting infinite monitor...", config.targets.len());
    ...
    loop {
        ...
        let enum_success = unsafe { EnumProcesses(pids_buffer.as_mut_ptr(), cb_size, &mut cb_needed) }.is_ok();
        if enum_success {
            ...
            for &pid in &pids_buffer {
                ...
                let desired_access = PROCESS_QUERY_INFORMATION.0 | PROCESS_SET_INFORMATION.0 | 0x1000u32;
                let h_query_result = unsafe { OpenProcess(PROCESS_ACCESS_RIGHTS(desired_access), BOOL(0), pid) };
                    ...
                    if name_len > 0 {
                        ...
                        for target in &config.targets {
                            if pname == target.name.to_lowercase() {
                                let h_set_result = unsafe { OpenProcess(PROCESS_ACCESS_RIGHTS(desired_access), BOOL(0), pid) };
                                if let Ok(handle) = h_set_result {
                                    ...
                                    if current_prio != target_prio {
                                        unsafe { let _ = SetPriorityClass(handle, PROCESS_CREATION_FLAGS(target_prio)); };
                                        println!("Set Priority to {} for '{}' (PID {})", target.priority, pname_str, pid);
                                    }

                                    // 2. Affinity
                                    let mut process_affinity: usize = 0;
                                    let mut system_affinity: usize = 0;
                                    if unsafe { GetProcessAffinityMask(handle, &mut process_affinity, &mut system_affinity) }.is_ok() {
                                        ...
                                        if process_affinity != target_mask {
                                            unsafe { let _ = SetProcessAffinityMask(handle, target_mask); };
                                            println!("Set Affinity to 0x{:x} for '{}' (PID {})", target_mask, pname_str, pid);
                                        }
                                    }

                                    // 3. IO Priority
                                    if let Some(io_prio_str) = &target.io_priority {
                                        if let Some(target_io_prio) = get_io_priority_hint(io_prio_str) {
                                            ...
                                            if ret.is_ok() && current_io_prio.0 != target_io_prio.0 {
                                                ...
                                                println!("Set IO Priority to {} for '{}' (PID {})", io_prio_str, pname_str, pid);
                                            }
                                        }
                                    }
                                    ...
                                    if let Some(mem_prio_str) = &target.memory_priority {
                                        if let Some(target_mem_prio_val) = get_memory_priority(mem_prio_str) {
                                            let mut current_mem_prio = MEMORY_PRIORITY_INFORMATION { MemoryPriority: MEMORY_PRIORITY(0) };
                                            let ret = unsafe {
                                                GetProcessInformation(
                                                    handle,
                                            ...
                                            if ret.is_ok() && current_mem_prio.MemoryPriority.0 != target_mem_prio_val {
                                                let target_mem_prio = MEMORY_PRIORITY_INFORMATION { MemoryPriority: MEMORY_PRIORITY(target_mem_prio_val) };
                                                unsafe {
                                                    let _ = SetProcessInformation(
                                                        handle,
                                                        PROCESS_INFORMATION_CLASS(39), // ProcessMemoryPriority
                                                ...
                                                println!("Set Memory Priority to {} for '{}' (PID {})", mem_prio_str, pname_str, pid);
                                            }
                                        }
                                    }
                                    ...
                                    if let Some(power_str) = &target.power_throttling {
                                        ...
                                        let mut current_power = PROCESS_POWER_THROTTLING_STATE { Version: 0, ControlMask: 0, StateMask: 0 };
                                        let ret = unsafe {
                                            GetProcessInformation(
                                                handle,
                                        ...
                                        let target_state_mask = if enable { PROCESS_POWER_THROTTLING_EXECUTION_SPEED } else { 0 };
                                        let current_state_bit = current_power.StateMask & PROCESS_POWER_THROTTLING_EXECUTION_SPEED;
                                        ...
                                        if ret.is_ok() && current_state_bit != target_state_mask {
                                            ...
                                            println!("Set Power Throttling to {} for '{}' (PID {})", power_str, pname_str, pid);
                                        }
                                    }
